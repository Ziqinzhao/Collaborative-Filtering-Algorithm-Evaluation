...
```{r }
#pairwise cosine distance  https://rdrr.io/cran/stylo/src/R/dist.cosine.R
dist.cosine = function(x){  # x =  matrix q (movies:10*2427)
    ...
    return(y) # y = 2427*2427
} # max high, simliar high
...
distance_movie<-dist.cosine(t(Q))%>%as.matrix() #+1.2*diag(I) # T:need transpose
rownames(distance_movie)<-Movie_all # 2427个
colnames(distance_movie)<-Movie_all # 2427个

for (u in 1:U){  # for every user (32 in total)                                          # only train dataset(since A/R part only contains train dataset)   
    Movie_rated_by_u<-train[train$userId == u,]$movieId%>%unique%>%as.character()        # find trainset: movie rated by u(all movies if rated by user u)
    Movie_NOTrated_by_u<-Movie_all[!Movie_all%in%Movie_rated_by_u]                       # all movies if not rated by user u
    Rating_by_u<-R[u,]                                                                   # predict rating of u
    
    for (i in Movie_NOTrated_by_u){ # for every movie not rated by u                       
        tmp<-distance_movie[i,Movie_rated_by_u]%>%which.max%>%names # find the highest dist.cosine score (which is the most simliarity)
        Rating_by_u[i]<-Rating_by_u[tmp]                            # using the most simliarity movie which is rated by u to replace the score of the movie which is                                                                     # not rated by u
    }
    R[u,]<-Rating_by_u 
